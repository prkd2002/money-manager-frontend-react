...
const IncomeList = ({transactions,onDelete, onDownload,onEmail, isLoadingDownload,isLoadingEmail}) => {
    return (
        <div className="card  ">
            ...
            <div className="grid grid-cols-1 gmd:grid-cols-2  ">
                ...
                {transactions?.map((transaction, index) => (
                    <TransactionInfoCard key={index} title={transaction.name} icon={transaction.icon} date={moment(transaction.date).format('DD-MM-YYYY')} amount={transaction.amount} type={"Income"} onDelete={() => onDelete(transaction.id)} />
                ))}
